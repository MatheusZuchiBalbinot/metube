...
describe('GroqClient::chat', function () {
    test('returns plain text answer', function () {
        Http::fake([
            '*' => Http::response([
                'choices' => [
        ...
        $client = app(GroqClient::class);
        $answer = $client->chat('What is this about?', 'System context here.', []);

        expect($answer)->toBe('The video is about Laravel.');
    });

    test('includes history in the request', function () {
        Http::fake([
            '*' => Http::response([
                'choices' => [
        ...
        $client = app(GroqClient::class);
        $answer = $client->chat('Anything else?', 'Context.', $history);
    ...
    test('throws on empty response', function () {
        Http::fake([
            '*' => Http::response([
                'choices' => [
                    ['message' => ['content' => '']],
        ...
        expect(fn () => $client->chat('Question?', 'Context.', []))
