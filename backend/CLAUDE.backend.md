...
Para ver o shape exato de cada resource, leia o arquivo PHP correspondente em `app/Http/Resources/`. Regras gerais:
- Datas sempre em ISO 8601 via `->toIso8601String()`
- Relações via `$this->whenLoaded(...)` — nunca force eager load dentro do resource
- `is_liked` em `CommentResource` é atributo virtual injetado pelo service (sem coluna no banco)
