...
class HlsTranscodeService
{
    ...
    /**
     * @return float|null Null when ffprobe cannot determine the duration
     */
    ...
    /**
     * One `-map 0:a` pair per rendition, so var_stream_map can pair v:N with a:N.
     *
     * @return list<string>
     */
    ...
    /**
     * Used to compute a suitable VBV buffer size (-bufsize:v:N). Supports "k" and
     * "M" suffixes (case-insensitive); returns the input unchanged otherwise.
     */
    ...
    /**
     * Public-disk-relative HLS directory (contrast with the absolute path from ensureHlsDirectory()).
     */
